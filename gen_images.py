...
def gen_image(prompt, output_path):
    ...
    data = resp.json()
    if "data" not in data:
        print(f"ERROR for {output_path}: {data}")
        return False
...
images = [
    ...
    (
        "images/ocean/issue001/q2-fish-formation.png",
        "underwater scene in a glowing coral city, four tight schools of silver fish weaving between coral towers, two fish in each school, clearly separated formations, vivid bioluminescent coral background, deep indigo water, no text"
    ),
    (
